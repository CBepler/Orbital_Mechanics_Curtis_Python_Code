...
from helpers import get_observer_position, degrees_to_radians
...
def alg5_4(
    ...
    declination = np.arcsin(
        np.cos(latitude) * np.cos(azimuth) * np.cos(elevation)
        + np.sin(latitude) * np.sin(elevation)
    )
    # Calculate topocentric right ascension
    hour_angle = np.arccos(
        (
            np.cos(latitude) * np.sin(elevation)
            - np.sin(latitude) * np.cos(elevation) * np.cos(azimuth)
        )
        / (np.cos(declination))
    )
    if azimuth < np.pi:
    ...
    declination_rate = (1 / np.cos(declination)) * (
        -azimuth_rate * np.cos(latitude) * np.sin(azimuth) * np.cos(elevation)
        + elevation_rate
        * (
            np.sin(latitude) * np.cos(elevation)
            - np.cos(latitude) * np.cos(azimuth) * np.sin(elevation)
        )
    )
    # Calculate the right ascension rate
    right_ascension_rate = ANGULAR_VELOCITY_EARTH + (
        azimuth_rate * np.cos(azimuth) * np.cos(elevation)
        - elevation_rate * np.sin(azimuth) * np.sin(elevation)
        + declination_rate * np.sin(azimuth) * np.cos(elevation) * np.tan(declination)
    ) / (
        np.cos(latitude) * np.sin(elevation)
        - np.sin(latitude) * np.cos(azimuth) * np.cos(elevation)
    )
    # Calculate the direction cosines rates vector
    direction_cosines_rate = np.array(
        [
            (-right_ascension_rate * np.sin(right_ascension) * np.cos(declination))
            - (declination_rate * np.cos(right_ascension) * np.sin(declination)),
            (right_ascension_rate * np.cos(right_ascension) * np.cos(declination))
            - (declination_rate * np.sin(right_ascension) * np.sin(declination)),
            declination_rate * np.cos(declination),
        ]
    )
...
if __name__ == "__main__":
    r, v = alg5_4(
        988,
        4.86,
        degrees_to_radians(36),
        degrees_to_radians(0.590),
        degrees_to_radians(36.6),
        degrees_to_radians(-0.263),
